...
# --- CloudFront Function (clean URLs / canonical host / https) ---
resource "aws_cloudfront_function" "mkdocs_router" {
  count   = local.need_router_fn ? 1 : 0
  name    = local.cf_function_name
  runtime = "cloudfront-js-2.0"
  publish = true
  code    = templatefile("${path.module}/cloudfront/mkdocs_router.tftpl", {
    canonical_host    = local.canonical_host_s
  ...
  default_cache_behavior {
    ...
    dynamic "function_association" {
      ...
      content {
        ...
        function_arn = aws_cloudfront_function.mkdocs_router[0].arn
      }
    }
  }
